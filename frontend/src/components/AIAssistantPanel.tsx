  ...
  return (
    <div className="bg-white rounded-lg p-6 border border-slate-200 h-full flex flex-col">
      <div className="flex items-center space-x-2 mb-6 border-b border-slate-200 pb-4">
        ...
        <h2 className="text-xl font-semibold text-slate-900">AI Review Assistant</h2>
        <span className="bg-purple-500/20 text-indigo-700 text-xs px-2 py-1 rounded">gemini-2.5-flash</span>
      </div>

      <div className="flex flex-wrap gap-2 mb-6">
        <button onClick={() => invokeAI("EXPLAIN")} className={`px-3 py-1.5 rounded text-sm flex items-center space-x-1 ${activeTab === 'EXPLAIN' ? 'bg-indigo-600 text-slate-900' : 'bg-slate-100 text-slate-600 hover:bg-slate-600'}`}>
          <AlertCircle className="w-4 h-4" /> <span>Explain</span>
        </button>
        <button onClick={() => invokeAI("SUGGEST")} className={`px-3 py-1.5 rounded text-sm flex items-center space-x-1 ${activeTab === 'SUGGEST' ? 'bg-indigo-600 text-slate-900' : 'bg-slate-100 text-slate-600 hover:bg-slate-600'}`}>
          <RefreshCcw className="w-4 h-4" /> <span>Suggest</span>
        </button>
        <button onClick={() => invokeAI("COMPARE")} className={`px-3 py-1.5 rounded text-sm flex items-center space-x-1 ${activeTab === 'COMPARE' ? 'bg-indigo-600 text-slate-900' : 'bg-slate-100 text-slate-600 hover:bg-slate-600'}`}>
          <ShieldAlert className="w-4 h-4" /> <span>Compare Sources</span>
        </button>
        <button onClick={() => invokeAI("NOTE")} className={`px-3 py-1.5 rounded text-sm flex items-center space-x-1 ${activeTab === 'NOTE' ? 'bg-indigo-600 text-slate-900' : 'bg-slate-100 text-slate-600 hover:bg-slate-600'}`}>
          <FileSignature className="w-4 h-4" /> <span>Gen Note</span>
        </button>
      </div>

      <div className="flex-1 bg-white rounded p-4 overflow-y-auto border border-slate-200">
        {loading && (
          <div className="flex flex-col justify-center items-center h-full text-slate-500 space-y-3">
            <Bot className="w-8 h-8 animate-pulse text-indigo-600" />
        ...
        {!loading && result && (
          <div className="space-y-4">
            ...
            {activeTab === "EXPLAIN" && (
              <>
                <div>
                  <h4 className="text-slate-500 text-xs font-semibold uppercase">Explanation</h4>
                  <p className="text-slate-900 mt-1">{result.data.explanation}</p>
                </div>
                <div className="flex space-x-4 mt-4">
                  <div>
                    <h4 className="text-slate-500 text-xs font-semibold uppercase">AI Severity</h4>
                    <span className="text-red-600 font-bold">{result.data.severity}</span>
                  </div>
                  <div>
                    <h4 className="text-slate-500 text-xs font-semibold uppercase">Confidence</h4>
                    <span className="text-blue-600">{result.data.confidence}</span>
                  </div>
                </div>
              </>
            )}
            ...
            {(activeTab === "SUGGEST" || activeTab === "COMPARE") && (
              <>
                <div>
                  <h4 className="text-slate-500 text-xs font-semibold uppercase">Analysis</h4>
                  <p className="text-slate-900 mt-1">{result.data.reason || result.data.analysis}</p>
                </div>
                <div className="mt-4">
                  <h4 className="text-slate-500 text-xs font-semibold uppercase mb-2">Suggested Value (Editable)</h4>
                  <input 
                    type="text" 
                    value={editedValue} 
                ...
                <div className="mt-6 border-t border-slate-200 pt-4 flex flex-col sm:flex-row gap-3">
                  ...
                  <button onClick={() => handleAction("REJECT")} className="flex-1 bg-red-600/20 hover:bg-red-600/40 text-red-600 py-2 rounded flex justify-center items-center">
            ...
            {activeTab === "NOTE" && (
              <>
                <div>
                  <h4 className="text-slate-500 text-xs font-semibold uppercase">Generated Note</h4>
                  <textarea 
                    value={editedValue || result.data.note}
                    onChange={(e) => setEditedValue(e.target.value)}
                    className="w-full h-32 bg-white border border-slate-300 rounded p-3 text-slate-900 mt-2"
                  />
                </div>
            ...
            <div className="text-xs text-slate-500 mt-6 pt-4 border-t border-slate-200">
