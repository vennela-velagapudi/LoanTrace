...
loans_with_multiple_excs = db.query(NormalizedLoan.id).join(ExceptionModel).group_by(NormalizedLoan.id).having(func.count(ExceptionModel.id) >= 2).all()
...
# 1. Assign EX2
ex2 = assign_reviewer(db, ex2.id, 2)
print(f"Assigned EX2, status is now {ex2.status}")

# 2. Make decision on EX2 (Resolve)
ex2 = make_decision(db, ex2.id, 2, "APPROVE", "looks good")
